...
import {
  ...
  enableUsbTcpip,
  getDeviceIpAddress,
  startQrPairingSession,
  WirelessPairPayload,
  WirelessConnectPayload,
  WirelessDisconnectPayload,
} from '../adb/connectionManager';
...
export function registerWirelessHandlers(): void {
  ...
  ipcMain.handle('adb:pair-wireless', async (_event, payload: WirelessPairPayload) => {
    try {
  ...
  ipcMain.handle('adb:connect-wireless', async (_event, payload: WirelessConnectPayload) => {
    try {
  ...
  ipcMain.handle('adb:disconnect-wireless', async (_event, payload: WirelessDisconnectPayload) => {
    try {
  ...
  ipcMain.handle('adb:enable-tcpip', async (_event, serial: string, port?: number) => {
    try {
  ...
  ipcMain.handle('adb:get-device-ip', async (_event, serial: string) => {
    try {
