...
import { getAdbPath, runAdb } from './adbManager.mjs';
import * as os from 'os';
let activePairWatcher = null;
/**
 * Gets the computer's primary local Wi-Fi / LAN IP address.
 */
export function getLocalComputerIp() {
 ...
 */
export async function startQrPairingSession() {
 ...
 */
export async function checkMdnsPairing() {
    try {
 ...
 */
export function detectConnectionType(serial) {
 ...
 */
export async function getDeviceIpAddress(serial) {
    try {
 ...
 */
export async function enableUsbTcpip(serial, port = 5555) {
 ...
 */
export async function pairWirelessDevice(payload) {
 ...
 */
export async function connectWirelessDevice(payload) {
 ...
 */
export async function disconnectWirelessDevice(payload) {
    const { host, port } = payload;
    const adbPath = await getAdbPath();
    const endpoint = `${host.trim()}:${port}`;
    return new Promise((resolve) => {
